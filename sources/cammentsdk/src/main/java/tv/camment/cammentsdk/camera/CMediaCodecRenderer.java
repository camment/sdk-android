...
import tv.camment.cammentsdk.exoplayer.BaseRenderer;
...
import tv.camment.cammentsdk.exoplayer.drm.DrmSession;
...
import tv.camment.cammentsdk.exoplayer.source.MediaPeriod;
import tv.camment.cammentsdk.exoplayer.util.Assertions;
import tv.camment.cammentsdk.exoplayer.util.NalUnitUtil;
import tv.camment.cammentsdk.exoplayer.util.TraceUtil;
import tv.camment.cammentsdk.exoplayer.util.Util;

public abstract class CMediaCodecRenderer extends BaseRenderer {
    ...
    private static final String TAG = "CMediaCodecRenderer";
     ...
     */
    public CMediaCodecRenderer(int trackType, MediaCodecSelector mediaCodecSelector,
    ...
    protected final void maybeInitCodec() throws ExoPlaybackException {
        ...
        if (codecInfo == null) {
            try {
                codecInfo = getMediaCodecInfo(format.sampleMimeType, drmSessionRequiresSecureDecoder);
                if (codecInfo == null && drmSessionRequiresSecureDecoder) {
                    ...
                    codecInfo = getMediaCodecInfo(format.sampleMimeType, false);
                    if (codecInfo != null) {
        ...
        try {
            Log.d(TAG, "Codec name: " + codecName + " " + this);
            ...
            if (codecInfos != null && codecInfos.size() > lastCodecInfoIndex + 1) {
                Log.d(TAG, "trying another one" + this);
                codecInfo = null;
                if (codec != null) {
                    codec.release();
                }
                ...
                Log.d(TAG, "throwDecoderInitError");
                throwDecoderInitError(new DecoderInitializationException(format, e,
                        drmSessionRequiresSecureDecoder, codecName));
            }
        }

        lastCodecInfoIndex = -1;
    ...
    private MediaCodecInfo getMediaCodecInfo(String mimeType, boolean drmSessionRequiresSecureDecoder) throws MediaCodecUtil.DecoderQueryException {
        codecInfos = MediaCodecUtil.getDecoderInfos(mimeType, drmSessionRequiresSecureDecoder);
        if (codecInfos != null && codecInfos.size() > lastCodecInfoIndex + 1) {
            codecInfo = codecInfos.get(lastCodecInfoIndex + 1);
        } else {
            codecInfo = null;
        }
        ...
        Log.d(TAG, "getMediaCodecInfo " + (this.codecInfo == null ? "null" : this.codecInfo.name) + " " + this);
