...
public final class PermissionHelper implements EasyPermissions.PermissionCallbacks {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        EventBus.getDefault().post(new PermissionStatusChangedEvent());
    }

    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == AppSettingsDialog.DEFAULT_SETTINGS_REQ_CODE) { //if permissions were granted from settings
            EventBus.getDefault().post(new PermissionStatusChangedEvent());
        }
    }
